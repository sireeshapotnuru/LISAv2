...
RunTest() {
	...
	# RPM utility in 6.X till 7.1 distro uses Free/Available space available to non-root user.
	# Where as RPM in >= 7.2 distro does not have this limitation and space available for root user in used.
	# In order to address the difference below condition is added.
	if [[ $distro_version == 6* || $distro_version == 70 || $distro_version == 71 || $distro == "oracle" ]];then
	...
	if [[ $test_type == "negative" ]]; then
		# RPM utility in 6.X till 7.1 distro has a minimum size check of 10 MB. RPM utility checks for non super user free space.
		# Even if we have a space for root user, RPM will not allow to proceed further. Hence setting the limit as 8MB to validate the negative case.
		if [[ $distro_version == 6* || $distro_version == 70 || $distro_version == 71 ]];then
			boot_part_required_space=8478720 #8M
			root_part_required_space=8478720 #8M
		else
	...
	if [[ $boot_partition == $root_partition ]]; then
		...
		echo "single_partition_created_file_size $single_partition_created_file_size"
